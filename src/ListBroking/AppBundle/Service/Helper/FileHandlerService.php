...
//        $this->exportByArray($path, $extension, $query->execute(null, Query::HYDRATE_ARRAY));
    ...
    public function generateFileFromArray ($name, $extension, $array, $zipped = true)
    {
        // Generate File
        $path  = $this->generateFilename($name, $extension, true, '/../web/exports/');
        $this->exportByArray($path, $extension, $array);
    ...
    public function saveFormFile (Form $form)
    {
        ...
        $path  = $this->generateFilename($uploaded_file->getClientOriginalName(), null, 'imports/');

        return $uploaded_file->move('imports', $path);
    }
